...
import { ROUTE_PATH, VIP_PACKAGE_LEVEL_ARRAY, VIP_PACKAGE_LEVEL_NUMERIC, VIP_PACKAGE_LEVEL_SHOWS } from '@/constants';
...
const Edit = () => {
  ...
  const { id } = useParams();
  const { editVipPackage, getVipPackageById } = useVipPackageService();

  const { data: vipPackage } = useQuery({
    queryKey: ['admin-vip-packages', id],
    queryFn: () => getVipPackageById(id!),
    enabled: !!id,
  });
  ...
  const form = useForm<VipPackageFormValues>({
    ...
    values: vipPackage ? {
      ...
      numPost: vipPackage.numPost,
      price: vipPackage.price,
      durationDay: vipPackage.durationDay,
      priority: String(VIP_PACKAGE_LEVEL_NUMERIC[vipPackage.priority])
    } : undefined,
  });

  const mutation = useMutation({
    mutationFn: (data: VipPackageFormValues) => editVipPackage(id!, data),
    onSuccess: () => {
      form.reset();
  ...
  return (
    <div className="space-y-6 dark:bg-black dark:text-white">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-3xl font-bold">Cập nhật gói VIP</h1>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          <FormItemCustom
            ...
            renderInput={({ value, onChange }) => (
              <Select key={value} value={String(value) || ''} onValueChange={onChange}>
                <SelectTrigger>
