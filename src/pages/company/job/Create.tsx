    ...
    return (
        <div className="space-y-6 dark:bg-black dark:text-white">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <h1 className="text-3xl font-bold">Thêm mới công việc</h1>
            </div>
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <div className='flex items-center gap-2'>
                        <Button type="submit" disabled={mutation.isPending} className="cursor-pointer bg-gradient-to-r from-cyan-600 to-teal-600 text-white shadow-lg hover:from-cyan-700 hover:to-teal-700">
                            {mutation.isPending ? 'Đang tạo mới...' : 'Tạo mới'}
                        </Button>

                        <Button type='button' className='cursor-pointer bg-gray-200 text-gray-800 hover:bg-gray-300' onClick={() =>
                            navigate(ROUTE_PATH.ADMIN.SKILLS.LIST)
                        }>Quay lại</Button>
                    </div>
                </form>
